...
locals {
  function_name = join("", [for element in split("-", lower(var.function_name)) : title(element)])
  ...
  alarm_name = var.alarm_name == null ? "${local.function_name}Lambda${var.statistic}${var.metric_name}${var.comparison}${var.threshold}In${var.evaluation_periods}PeriodsOf${var.period}" : var.alarm_name
}
...
resource "opsgenie_api_integration" "opsgenie_integration" {
  name = "Terraform${data.aws_caller_identity.default.account_id}LambdaIntegration${local.alarm_name}"
  type = "AmazonSns"
...
resource "aws_cloudwatch_metric_alarm" "alarm" {
  ...
  # Link to the requested Lambda instance
  dimensions = {
